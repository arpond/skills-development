...
This file covers the core loop only. Two optional subsystems live in their own files and are
...
**If it doesn't exist yet, create it before doing anything else** -- this is part of the skill,
not a separate setup step the user has to remember to do first:
...
3. **Interrogate this with the user before writing anything -- don't just show a list and wait
   ...
   that rule inline (see the tracker format above).
...
5. Write the file with the confirmed tiers, `Selection strategy:`, `Feedback:` setting, and
   today's date as `Last reviewed:`, then proceed to Step 1 (it will find the categories empty,
   which is an expected and normal case of "running low," not an error).
...
a single short question, not a repeat of the full Step 0 interrogation -- don't re-litigate every
tier from scratch unless the user wants to.
