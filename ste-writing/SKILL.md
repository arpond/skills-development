...
**Hard rules, indexed so a review can check none have gone missing.** Scope: rules that fail
*silently* if skipped — the output reads as clean STE with the rule broken. A dropped number, a
restyled error string, or an unlinted draft reported as clean all look fine on the page. Rules
that fail visibly are deliberately absent — a semicolon left in, a sentence over the cap, or a
contraction all show up in the text or in the lint score. Absence from this table does not mean
optional.

| Where | Hard rule |
|---|---|
| Guards | Never drop a fact, number, condition, or scope qualifier to satisfy a length cap — keep the longer sentence and flag it |
| Guards | Preserve code identifiers, part numbers, units, error strings, and safety wording exactly |
| Guards | If the input already complies, return it unchanged and say so — do not restyle text a rule does not touch |
| Three ways (review) | In review mode, do not rewrite — output the violation table only |
| Verify | When commands can run, do not present text as clean without a lint run |
| Scope | Do not paste the full standard — it is copyrighted; link to it instead |
...
If you can run commands, lint the draft with `scripts/ste-lint.py`, which lives in this skill's
folder (the directory that holds this SKILL.md). Write the draft to a temporary file first, then:

```
python <skill-dir>/scripts/ste-lint.py draft.md            # flavored target: under 2.5 per 100 words
python <skill-dir>/scripts/ste-lint.py --strict draft.md   # strict target: under 1.5 per 100 words
```

The script needs Python 3 and nothing outside the standard library. It reads the file and prints
a score. It never writes. `--json` gives the full breakdown. `--fail-over N` exits 1 when the
worst file scores above N per 100 words.
